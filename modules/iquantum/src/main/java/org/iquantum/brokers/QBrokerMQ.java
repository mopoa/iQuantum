...
import org.iquantum.policies.qtasks.SubgraphIsomorphism;
import java.util.*;
...
public class QBrokerMQ extends SimEntity {
    ...
    //  checking the subGraph Isomorphism
    private QNodeMQ selectBestQNode(QTask Qtask) {
        QNodeMQ qn=null;
        for (int i=0; i< getQNodeList().get(0).getQPUList().getQubitTopologyOfQPUById(i).getQubits().size(); i++) {
            // درک بهتر qpu list
            if(SubgraphIsomorphism.isSubgraph(Qtask,getQNodeList().get(i).getQPUList().getQubitTopologyOfQPUById(i).getQubits())){
                bindQTaskToQNode(Qtask.getQTaskId(),getQNodeList().get(i).getId());
                qn= getQNodeList().get(i);
                break;
            }
...
//            }
            if (QTask.getQNodeId() == -1) {
                ...
                qNode =getQNodeList().get(QTask.getQNodeId());
                if (qNode == null) {
                    if(!Log.isDisabled()) {
                        Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Postponing execution of QTask ", QTask.getQTaskId(),
                                ": QNode is not available");
